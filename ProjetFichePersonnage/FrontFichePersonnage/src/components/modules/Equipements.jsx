...
export default function Equipements({ equipements, idFiche, onUpdate, suggestions = [], lectureSeule = false }) {
  ...
  const ajouterDepuisExistant = async (item) => {
    setErreur('')
    try {
      await api.ajouterEquipement(idFiche, item)
      onUpdate()
    } catch (err) {
      setErreur(err.message)
    }
      ...
      {!collapsed && (
        <div style={{ padding: '16px' }}>
          ...
          {!lectureSeule && (
            <SelecteurExistants
              items={suggestions}
              dejaPresents={equipements.liste.map(e => e.nom ?? e)}
